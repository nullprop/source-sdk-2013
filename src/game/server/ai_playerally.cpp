...
bool CAI_PlayerAlly::SelectSpeechResponse( AIConcept_t conc, const char *pszModifiers, CBaseEntity *pTarget, AISpeechSelection_t *pSelection )
{
	...
	{
		bool result = SpeakFindResponse( pSelection->Response, conc, pszModifiers );
		if ( result )
		{
			pSelection->conc = conc;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_PlayerAlly::SetPendingSpeech( AIConcept_t conc, AI_Response &Response )
{
...
void CAI_PlayerAlly::PostSpeakDispatchResponse( AIConcept_t conc, AI_Response *response )
{
	...
	if ( pConceptInfo && (pConceptInfo->flags & AICF_QUESTION) && GetSpeechTarget() )
	{
		...
		if ( rr_debug_qa.GetBool() )
		{
			if ( bSaidHelloToNPC )
			{
				Warning("Q&A: '%s' said Hello to '%s' (concept %s)\n", GetDebugName(), GetSpeechTarget()->GetDebugName(), conc );
			}
			else
			{
