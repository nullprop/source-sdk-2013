...
// Output : AI_Response
//-----------------------------------------------------------------------------
bool CAI_Expresser::SpeakFindResponse( AI_Response &outResponse, AIConcept_t conc, const char *modifiers /*= NULL*/ )
{
...
// Input  : *response - 
//-----------------------------------------------------------------------------
bool CAI_Expresser::SpeakDispatchResponse( AIConcept_t conc, AI_Response& response, IRecipientFilter *filter /* = NULL */ )
{
	bool spoke = false;
	...
	if ( IsSpeaking() && conc[0] != 0 )
	{
		DevMsg( "SpeakDispatchResponse:  Entity ( %i/%s ) already speaking, forcing '%s'\n", GetOuter()->entindex(), STRING( GetOuter()->GetEntityName() ), conc );
	...
	if ( spoke )
	{
		...
		if ( DebuggingSpeech() && g_pDeveloper->GetInt() > 0 && response.GetType() != RESPONSE_PRINT )
		{
			Vector vPrintPos;
			GetOuter()->CollisionProp()->NormalizedToWorldSpace( Vector(0.5,0.5,1.0f), &vPrintPos );
			NDebugOverlay::Text( vPrintPos, CFmtStr( "%s: %s", conc, szResponse ), true, 1.5 );
		}
...
bool CAI_Expresser::Speak( AIConcept_t conc, const char *modifiers /*= NULL*/, char *pszOutResponseChosen /* = NULL*/, size_t bufsize /* = 0 */, IRecipientFilter *filter /* = NULL */ )
{
	...
	bool result = SpeakFindResponse( response, conc, modifiers );
	if ( !result )
		return false;

	SpeechMsg( GetOuter(), "%s (%p) spoke %s (%f)\n", STRING(GetOuter()->GetEntityName()), GetOuter(), conc, gpGlobals->curtime );

	bool spoke = SpeakDispatchResponse( conc, response, filter );
